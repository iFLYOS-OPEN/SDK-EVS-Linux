...
public:
     ...
     *
     * @param ext2
     *        extra message, for MEDIA_PLAYING_STmAppNameATUS means decoded frame time.
     *
     * @param fromThread
     *        from rplayer or not.
     *
     */
    virtual void notify(long sid, int msg, int ext1, int ext2, int fromThread) = 0;
};
...
public:
     ...
     *
     * @param cacheModevoid MediaPlayer::notifyListener(void *clazz, int msg, int ext1, int ext2, int fromThread)
     ...
     *         return the status, 0 : success, < 0 : failure.
     */
    status_t setDataSource(const std::string& url, const long sid);
