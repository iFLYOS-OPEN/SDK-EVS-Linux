...
static int eventListener(void *opaque, int what, int arg1, int arg2, int from_thread)
{
    ...
    {
        case MEDIA_PREPARED: {
            //auto seekRetStatus = mp->seekTo(30000);
            //printf("SeekTo after prepared:%d\n", seekRetStatus);
        }
            break;
        case MEDIA_STOPED:
        case MEDIA_ERROR:
...
public:
    ...
    void notify(long sid, int msg, int arg1, int arg2, int fromThread)
    {
