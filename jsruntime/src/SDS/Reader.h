...
ssize_t SharedDataStream<T>::Reader::read(void* buf, size_t nWords, std::chrono::milliseconds timeout) {
    if (nullptr == buf) {
        ...
        std::cerr << "SDS readFailed>nullBuffer" << std::endl;
        return Error::INVALID;
    }

    if (0 == nWords) {
        //logger::acsdkError(logger::LogEntry(TAG, "readFailed").d("reason", "invalidNumWords").d("numWords", nWords));
        std::cerr << "SDS readFailed>invalidNumWords: " << nWords << std::endl;
        return Error::INVALID;
    }
...
//                                       .d("readerCursor", m_readerCursor->load()));
                std::cerr << "SDK seekFailed(seekBeforeStreamStart)>reference:BEFORE_READER,seekOffset:" << offset <<
                ",readerCursor:" << m_readerCursor->load() << std::endl;
                return false;
            }
            absolute = *m_readerCursor - offset;
            break;
...
//                                       .d("writeStartCursor", writeStartCursor->load()));
                std::cerr << "SDK seekFailed(seekBeforeStreamStart)>reference:BEFORE_WRITER,seekOffset:" << offset <<
                          ",readerCursor:" << m_readerCursor->load() << std::endl;
                return false;
            }
...
//                               .d("readerCloseIndex", m_readerCloseIndex->load()));
        std::cerr << "SDK seekFailed(seekBeyondCloseIndex)>position:" << absolute <<
                  "readerCursor:" << m_readerCursor->load() << std::endl;
        return false;
    }
    ...
    if (*writeEndCursor >= absolute && *writeEndCursor - absolute > m_bufferLayout->getDataSize()) {
        ...
        std::cerr << "SDS seekFailed(seekOverwrittenData)" << std::endl;
        return false;
    }
...
typename SharedDataStream<T>::Index SharedDataStream<T>::Reader::tell(Reference reference) const {
    ...
    std::cerr << "SDS tellFailed(invalidReference)" << std::endl;
...
//                                       .d("offset", offset)
//                                       .d("writeStartCursor", writeStartCursor->load()));
                std::cerr << "SDK closeFailed (invalidIndex)>reference:BEFORE_WRITER" << ",offset:" << offset <<
                          ",writeStartCursor:" << writeStartCursor->load() << std::endl;
            } else {
                absolute = *writeStartCursor - offset;
            }
            validReference = true;
    ...
    if (!validReference) {
        ...
        std::cerr << "SDK closeFailed (invalidReference)" << std::endl;
    }
...
std::string SharedDataStream<T>::Reader::errorToString(Error error) {
    ...
    std::cerr << "SDS errorToStringFailed(invalidError)>" << "error:" << error << std::endl;
