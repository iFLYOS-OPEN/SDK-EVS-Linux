...
//                               .d("wordSizeLimit", std::numeric_limits<decltype(Header::wordSize)>::max()));
        std::cerr << "SDS initFailed(wordSizeTooLarge)>" << "wordSize:" << wordSize << ",wordSizeLimit:" <<
                  std::numeric_limits<decltype(Header::wordSize)>::max() << std::endl;
        return false;
    }
...
//                               .d("maxReadersLimit", std::numeric_limits<decltype(Header::maxReaders)>::max()));
        std::cerr << "SDS initFailed(wordSizeTooLarge)>" << "maxReadersTooLarge:" << maxReaders << ",maxReadersLimit:" <<
                  std::numeric_limits<decltype(Header::maxReaders)>::max() << std::endl;
        return false;
    }
...
//                               .d("expectedMagicNumber", std::to_string(MAGIC_NUMBER)));
        std::cerr << "SDS attachFailed(magicNumberMismatch)>" << "magicNumber:" << header->magic << ",expectedMagicNumber:" <<
                  std::to_string(MAGIC_NUMBER) << std::endl;
        return false;
    }
...
//                               .d("expectedVersion", std::to_string(VERSION)));
        std::cerr << "SDS attachFailed(incompatibleVersion)>" << "version:" << header->version << ",expectedVersion:" <<
                  std::to_string(VERSION) << std::endl;
        return false;
    }
...
//                               .d("expectedHash", stableHash(T::traitsName)));
        std::cerr << "SDS attachFailed(traitsNameHashMismatch)>hash:" << header->traitsNameHash << ",expectedHash:" <<
                  stableHash(T::traitsName) << std::endl;
        return false;
    }
    ...
    if (0 == header->referenceCount) {
        ...
        std::cerr << "SDS attachFailed(zeroUsers)" << std::endl;
        return false;
    }
...
//                               .d("maxNumUsers", std::numeric_limits<decltype(header->referenceCount)>::max()));
        std::cerr << "SDS attachFailed(bufferMaxUsersExceeded)>" << "numUsers:" << header->referenceCount << ",maxNumUsers:" <<
                  std::numeric_limits<decltype(header->referenceCount)>::max() << std::endl;
        return false;
    }
