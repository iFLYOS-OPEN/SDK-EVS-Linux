...
size_t SharedDataStream<T>::calculateBufferSize(size_t nWords, size_t wordSize, size_t maxReaders) {
    if (0 == nWords) {
        ...
        std::cerr << "SDS calculateBufferSizeFailed(numWordsZero)" << std::endl;
        return 0;
    } else if (0 == wordSize) {
        //logger::acsdkError(logger::LogEntry(TAG, "calculateBufferSizeFailed").d("reason", "wordSizeZero"));
        std::cerr << "SDS calculateBufferSizeFailed(wordSizeZero)" << std::endl;
        return 0;
    }
...
std::unique_ptr<SharedDataStream<T>> SharedDataStream<T>::create(
    ...
    if (0 == expectedSize) {
        ...
        std::cerr << "SDS createFailed(nullBuffer)" << std::endl;
        return nullptr;
...
//                               .d("expectedSize", expectedSize));
        std::cerr << "SDS createFailed(bufferSizeTooSmall)>bufferSize:" << buffer->size() <<
                  ",expectedSize:" << expectedSize << std::endl;
        return nullptr;
    }
...
//                               .d("forceReplacement", "false"));
        std::cerr << "SDS createWriterFailed(existingWriterAttached)>forceReplacement:false" << std::endl;
        return nullptr;
    } else {
...
std::unique_ptr<typename SharedDataStream<T>::Reader> SharedDataStream<T>::createReader(
    ...
    std::cerr << "SDS createReader(noAvailableReaders)" << std::endl;
    return nullptr;
}
...
//                               .d("readerId", id)
//                               .d("forceReplacement", "false"));
        std::cerr << "SDS createReaderLockedFailed(readerAlreadyAttached)>readerId:" << id << ",forceReplacement:false" << std::endl;
        return nullptr;
    } else {
