...
static jerry_value_t iotjs_websocket_check_error(uint8_t code) {
  switch (code) {
    case WS_ERR_INVALID_UTF8: {
      return jerry_get_value_from_error(JS_CREATE_ERROR(COMMON, "Invalid UTF8 string in UTF8 message"), true);
    }

    case WS_ERR_INVALID_TERMINATE_CODE: {
      return jerry_get_value_from_error(JS_CREATE_ERROR(COMMON, "Invalid terminate code received"), true);
    }

    case WS_ERR_UNKNOWN_OPCODE: {
      return jerry_get_value_from_error(JS_CREATE_ERROR(COMMON, "Uknown opcode received"), true);
    }
    ...
    case WS_ERR_FRAME_SIZE_LIMIT: {
      return jerry_get_value_from_error(JS_CREATE_ERROR(COMMON, "Frame size received exceeds limit"), true);
    }
...
JS_FUNCTION(ParseHandshakeData) {
  ...
  if (buff_wrap->length < 12 || strncmp(buff_wrap->buffer + 9, "101", 3)) {
    //printf("ParseHandshakeData:%d,%s\n", buff_wrap->length, buff_wrap->buffer);
    return iotjs_jval_create_error_without_error_flag("WebSocket connection failed");
  ...
  if(key_pos < buff_wrap->buffer){
      char ws_accept2[] = "sec-websocket-accept: ";
      key_pos = strstr(buff_wrap->buffer, ws_accept2) + strlen(ws_accept2);
  }
  char key[28] = { 0 };
  ...
  if (!iotjs_check_handshake_key(key, jsref)) {
    return iotjs_jval_create_error_without_error_flag("WebSocket handshake key comparison failed");
  }
...
static uint8_t iotjs_websocket_decode_frame(iotjs_wsclient_t *wsclient,
  ...
  switch (opcode) {
    case WS_OP_CONTINUE: {
      //printf("!!!!WS CONTINUE, current length:%d, new length:%d\n", wsclient->ws_buff.length, payload_len);
      ...
      if (fin_bit) {
        //printf("!!!WS FIN, firstbyte:%d, total length:%d\n", wsclient->ws_buff.first_byte, wsclient->ws_buff.length);
        uint8_t ret_val =
    ...
    default:
      fprintf(stderr, "unknown websocket opcode:%d\n", opcode);
...
//  printf("WsReceive. buf len:%d, cur len:%d, first_byte:%x\n"
//          , wsclient->tcp_buff.length,buffer_wrap->length,
//          wsclient->tcp_buff.length > 0 ? wsclient->tcp_buff.buffer[0] : buffer_wrap->buffer[0]);
  ...
  while (true) {
    ...
    printf("payload_byte:%x, mask:%d\n", current_buffer[1], mask);

    int current_buffer_roll_back = 0;
    current_buffer += 2;
    current_buffer_roll_back -= 2;
    ...
    if (!(payload_byte ^ WS_TWO_BYTES_LENGTH)) {
      ...
      current_buffer_roll_back -= (int)sizeof(uint16_t);
      ...
      if (payload_64bit_len > UINT32_MAX) {
        //printf("8,");
        return iotjs_websocket_check_error(WS_ERR_FRAME_SIZE_LIMIT);
      }
      payload_len = (uint32_t)payload_64bit_len;

      current_buffer += sizeof(uint64_t);
      current_buffer_roll_back -= (int)sizeof(uint64_t);
    } else {
      //printf("9,");
    ...
    if (mask && ((current_buffer + 4 > current_buffer_end) ||
      ...
      current_buffer += current_buffer_roll_back;
      //printf("10,");
      break;
