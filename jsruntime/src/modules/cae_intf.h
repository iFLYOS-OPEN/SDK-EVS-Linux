...
typedef void * VOIP_HANDLE;

//audioData 通话音频地址
//audioLen  通话音频字节数
//userData 用户回调数据
typedef void (*voip_audio_fn)(const void *audioData, unsigned int audioLen, int param1, const void *param2, void *userData);

#ifdef __cplusplus
extern "C" {
#endif /* __cplusplus */

/* 初始化实例(实例地址、资源地址、通话音频回调、预留参数、用户回调数据) */
int VOIPNew(VOIP_HANDLE *cae, const char* resPath, voip_audio_fn voipCb, const char *param, void *userData);
typedef int (* Proc_VOIPNew)(VOIP_HANDLE *cae, const char* resPath, voip_audio_fn voipCb, const char *param, void *userData);

/* 写入音频数据(实例地址、录音数据地址、录音数据长度) */
int VOIPAudioWrite(VOIP_HANDLE cae, const void *audioData, unsigned int audioLen);
typedef int (* Proc_VOIPAudioWrite)(VOIP_HANDLE cae, const void *audioData, unsigned int audioLen);

/* 销毁实例(实例地址) */
int VOIPDestroy(VOIP_HANDLE cae);
typedef int (* Proc_VOIPDestroy)(VOIP_HANDLE cae);
