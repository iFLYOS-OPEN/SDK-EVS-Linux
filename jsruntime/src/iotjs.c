...
#include "iflyos/IFLYOSJerryScript.h"
...
static bool jerry_initialize(iotjs_environment_t* env) {
  ...
  {//iflyos platform and product
    jerry_value_t global = jerry_get_global_object();
    jerry_value_t platform = jerry_create_string_from_utf8((const jerry_char_t *)IFLYOS_PLATFORM);
    iotjs_jval_set_property_jval(global, IOTJS_MAGIC_STRING_IFLYOS_PLATFORM, platform);
    jerry_release_value(platform);

    jerry_value_t product = jerry_create_string_from_utf8((const jerry_char_t *)IFLYOS_PRODUCT);
     iotjs_jval_set_property_jval(global, IOTJS_MAGIC_STRING_IFLYOS_PRODUCT, product);
    jerry_release_value(product);
