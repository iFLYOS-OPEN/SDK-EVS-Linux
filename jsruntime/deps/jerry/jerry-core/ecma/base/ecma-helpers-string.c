 ...
 */
ecma_string_t *
ecma_new_ecma_string_from_utf8 (const lit_utf8_byte_t *string_p2, /**< utf-8 string */
                                lit_utf8_size_t string_size) /**< string size */
{
  JERRY_ASSERT (string_p2 != NULL || string_size == 0);
  //JERRY_ASSERT (lit_is_valid_cesu8_string (string_p, string_size));
  lit_utf8_byte_t *string_p = (lit_utf8_byte_t*)string_p2;
  if(!lit_is_valid_cesu8_string (string_p, string_size)){
      printf("string_size:%d> ", string_size);
      for(unsigned int i = 0; i < string_size; i++){
          printf("%02x %c ", string_p[i], string_p[i]);
      }
      printf("\n wrong UTF8!!!!\n");
      //JERRY_ASSERT(0);
#define NOT_CRASH_STR "INVALID UTF8"
      string_p = (lit_utf8_byte_t *)NOT_CRASH_STR;
      string_size = 12;
  }
